...
<?php
        ...
        $logId = $_SESSION['id'];
        $sql = "SELECT * FROM `ads` JOIN adimg ON `ads`.id = `adimg`.`ads_id` WHERE `ads`.`user_id` = $logId GROUP BY `ads`.`id`" ;
        $rows = getResult($sql);
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                ...
                <table class="table table-striped table-dark">
                    ...
                    <a href="editAds.php?edit=<?=$row['id']?>">
                        <input type="button" value="Edit" class="btn btn-primary">
                    </a>    
               
                <button class="btn viewData btn-info" data-ad-value="<?php echo $row['id']?>">View</button>
                <button class="btn delete btn-danger"  data-ad-value="<?php echo $row['id']?>">Delete</button>
            </div>
        </div><!---row-->
    </div>
